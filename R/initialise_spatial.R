...
create_cell_data_csu_table = function(cmip6_calendars) {
  ...
  # code NaN to 0
  raster_table[is.nan((fertN.amt_mai)), fertN.amt_mai := 0][is.nan(orgN.amt), orgN.amt := 0][is.nan(orgCN.ratio), orgCN.ratio := 0][is.nan(res.rtrn.amt), res.rtrn.amt := 0]
  ...
  # code NaN to 0
  raster_table[is.nan((fertN.amt_soy)), fertN.amt_soy := 0][is.nan(orgN.amt), orgN.amt := 0][is.nan(orgCN.ratio), orgCN.ratio := 0][is.nan(res.rtrn.amt), res.rtrn.amt := 0]
  ...
  # code NaN to 0
  raster_table[is.nan((fertN.amt_whe)), fertN.amt_whe := 0][is.nan(orgN.amt), orgN.amt := 0][is.nan(orgCN.ratio), orgCN.ratio := 0][is.nan(res.rtrn.amt), res.rtrn.amt := 0]
  ...
  # add swheat
  setorder(main_table, gridid) # group crop.100 params
  ...
  main_table     = main_table[!is.na(gridid.rotated),] # remove after swheat added


  fwrite(main_table, paste(pkg.env$out_path,"cell_data_table_csu-22Dec2022.csv", sep = "/")) #csv
  save(main_table, file = paste(pkg.env$out_path,"cell_data_table_csu-22Dec2022.RData", sep = "/")) #rda
  return(main_table)
}
